...
create.strat.flg.f.YFT = function(lat.5deg,lon.5deg,is.lwrght,month,setype,vessel.class,PS) {
  ...
  area[lon.5deg>(-128.5)&lat.5deg>=(22.5)]<-5
  ...
  area<-rep(1,nrecs)
  area[lon.5deg>(-130) & lat.5deg>=5]<-3
  area[lon.5deg>(-130) & lat.5deg<5]<-2
  ...
  print("Using fishery stratification: YFT SAC 15 UNA")
  fishery.areagear<-rep(NA,nrecs)
  ...
  print("Using fishery stratification: YFT SAC 15 OBJ")
  fishery.areagear<-rep(NA,nrecs)
